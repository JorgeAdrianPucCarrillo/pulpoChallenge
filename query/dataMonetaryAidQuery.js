...
class dataMonetaryHelpQuery {
    async dataMonetaryAidJson(countryId, year) {
        try{
            let lessTime = 0
            //verificando fecha ------------------------------------------
            try{
            ...
            try{
                ...
                const resp = sortInfo.sortInformationAsJson(response.data.response, year)
                let saveCache = {}
    ...
    async dataMonetaryAidArray(countryId, year) {
        try{
            let lessTime = 0
            //verificando fecha ------------------------------------------
            try{
            ...
            try{
                const response = await axios.get(hostUrls, tockenHeader)
                const resp = sortInfo.sortInformationAsArray(response.data.response, year)
                let saveCache = {}
                saveCache[countryId+year]=resp
                expireCache.set(saveCache,10)
                return msg.msgSuccess(200,'exito',resp)
